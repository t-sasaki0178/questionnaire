...
Private Sub コメント用枠つくり(MakeSheetname As String, Total As Single, QTitle As String, QTitles() As Variant, _
 TrimAllText As Variant, Title_MinRow As Integer, Table_MinRow As Integer, MinCol As Integer)
    ...
    Range(Cells(Title_MinRow, MinCol + 1), Cells(Title_MinRow, MaxCol)).MergeCells = True
    ...
    Cells(Title_MinRow, MinCol + 1) = Format(QTitles(0))
    ...
    For i = 1 To UBound(QTitles())
        Cells(Title_MinRow + 1, MinCol + i) = QTitles(i)
    
         With Cells(Title_MinRow + 1, MinCol + i).Font
            .Name = "ＭＳ Ｐゴシック"
            .Size = 8
        End With
    
        With Cells(Title_MinRow + 1, MinCol + i)
            .HorizontalAlignment = xlLeft
            .VerticalAlignment = xlCenter
        End With
    Next
    ...
    '掲載欄のセンタリング
    'With Range(Cells(Table_MinRow, MinCol + 1), Cells(MaxRow, MinCol))
    '    .HorizontalAlignment = xlCenter
    '    .VerticalAlignment = xlCenter
    'End With
