...
import math
# 関連モジュール
import Q9_select_one
import Q9_select_two
...
def graph_chart_text(select):
    ...
    # 画像の出力
    plt.savefig('./save_images/Q9_circle.png',dpi=300,orientation='portrait',transparent=False,pad_inches=0.0,bbox_inches='tight')
    ...
    with open('jinja2_templetes/templetes_Q9.html',mode='w') as f:
        f.write(str(html))
    ...
    driver.get('file:///Users/t_sasaki/Documents/%E6%A5%AD%E5%8B%99/%E3%82%A2%E3%83%B3%E3%82%B1%E3%83%BC%E3%83%88/%E3%82%AB%E3%83%9F%E3%82%AA%E3%83%B3/macro_mk/jinja2_templetes/templetes_Q9.html')
    ...
    # テキスト画像の作成
    Q9_select_one.Q9_select_one(select)
    Q9_select_two.Q9_select_two(select)
    ...
    # 貼り付ける画像の読み込み
    fnms = ['./save_images/Q{}_circle.png'.format(select),'./save_images/Q{}_radio_chart.png'.format(select),'save_images/Q{}_select_one.png'.format(select),'save_images/Q{}_select_two.png'.format(select)]
    # ファイルでループさせる
    for fnm in fnms:
        # スライドの追加。６は白紙のスライド
