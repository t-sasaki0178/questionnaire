...
# 画像の出力
plt.savefig('./save_images/Q13_radio_graph.png',dpi=300,orientation='portrait',transparent=False,pad_inches=0.0,bbox_inches='tight')

# サブパターン：表の作成をHTMLで行う
# jinja2テンプレート出力
env = Environment(loader=FileSystemLoader('./', encoding='utf8'))
tmpl = env.get_template('jinja2_templetes/templetes_Q13.tmpl')
# 商品情報を入れるリストを宣言、初期化
items = []
# 回答割合データの作成（小数点一桁）
percent = []
...
with open('jinja2_templetes/templetes_Q13.html',mode='w') as f:
    f.write(str(html))
...
driver.get('file:///Users/t_sasaki/Documents/%E6%A5%AD%E5%8B%99/%E3%82%A2%E3%83%B3%E3%82%B1%E3%83%BC%E3%83%88/%E3%82%AB%E3%83%9F%E3%82%AA%E3%83%B3/macro_mk/jinja2_templetes/templetes_Q13.html')
page_width = driver.execute_script('return document.body.scrollWidth')
page_height = driver.execute_script('return document.body.scrollHeight')
driver.set_window_size(page_width,page_height)
driver.save_screenshot('save_images/Q13_radio_chart.png')
driver.quit()
...
# plt.axis('off')
# alls = len(act_data)
# # select_dataでkey毎の回答数は出しているのでそれを使う
# lists = []
# for key_item,value_item in select_data.items():
#     lists.append([(value_item/alls)*100,value_item])
# # forループだとnumpy形式のlistに追加できなかったのでリストとして作成後にnp.array()に適応させる
# to_nparray = np.array(lists)
# # 小数点の桁数がめちゃくちゃ長くなるパターンがあったので小数点一桁で表示を固定numpyのround()関数
# to_nparray = np.round(to_nparray,1)
# # 内包表記で記述短縮
# indexes = [nl for nl in key_data]
# columns = ['割合（％）','実数']
# plt.table(cellText=to_nparray,colLabels=columns,rowLabels=indexes,loc='upper center')
...
# 貼り付ける画像の読み込み
fnms = ['./save_images/Q13_radio_graph.png','./save_images/Q13_radio_chart.png']
# ファイルでループさせる
for fnm in fnms:
    # スライドの追加。６は白紙のスライド
