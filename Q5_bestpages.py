...
def checkbox(select):
    ...
    # save_lltext（選択肢の一覧データ）は文章をカットするのでカットされてない状態の文章も保存しておく
    save_lltext = []
    save_nocut_lltext = []
    for lltext in ll:
    ...
    # nocut_text:save_lltextから不要なデータを除いたもの（純選択肢データのみ）
    nocut_text = []
    th_counter = 0
    for snll in save_nocut_lltext:
        for snll_text in snll:
            # 前の方にいらないデータが集まっているので、ここで弾く
            if th_counter > 5:
    ...
    for reldata in f_outhead:
        if 'Q{}_'.format(select) in reldata:
            rel_number.append(rel_numcounter)
        rel_numcounter+=1

    # rel_number（設問番号）とkey_data（設問テキスト）から対応表を作る
    kv_list = {}
    for key,value in zip(range(rel_number[0],rel_number[-1]),key_data):
        kv_list[key] = value

    # 取得した関連データの位置から回答一覧を取得（各１回答ではない）
    act_data = []
    for act in f:
        act_num = 0
        for act_val in range(rel_number[0],rel_number[-1]):
            if int(act[act_val]) == 1:
                act_data.append(key_data[act_num])
            act_num+=1
    act_len = len(act_data)
    ...
    # １行１列のグラフの描画（ここまで合ってる）
    nrow = 1
    ncol = 1
    # col:横、row:縦
    # 選択肢の数が多い場合、選択肢数の1/3くらいでちょうど良さそう
    plt.figure(figsize=(6*ncol,round(len(key_data)/3)*nrow))
    # １つ目のsubplot領域にプロット
    ...
    tmpl = env.get_template('jinja2_templetes/templetes_Q5.tmpl')
    # 商品情報を入れるリストを宣言、初期化
    items = []
    # 回答割合・実数データの作成
    percent = []
    ...
    for key_item,value_item in select_data.items():
        sd_act.append(value_item)
        percent.append(round((value_item/act_len)*100,1))
    ...
    # 末尾にまとめの値を追加する
    items.append({'LABEL':'合計','ANSER_ONE':act_len,'ANSER_TWO':100})
    html = tmpl.render({'title':graph_title,'item':items})
    with open('jinja2_templetes/templetes_Q5.html',mode='w') as f:
        f.write(str(html))
    ...
    driver.get('file:///Users/t_sasaki/Documents/questionnaire/jinja2_templetes/templetes_Q5.html')
