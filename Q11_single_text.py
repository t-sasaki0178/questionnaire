...
def singletext(select):
        ...
        if ('Q{}'.format(select) in item[1]) == True:
            try:
                if math.isnan(item[4]):
                    l_title = 'Q{}'.format(select) + item[5]
            except:
                l_title = 'Q{}'.format(select) + item[4]
    ...
    tmpl = env.get_template('jinja2_templetes/templetes_one.tmpl')
    # 商品情報を入れるリストを宣言、初期化
    items = []
    ...
    with open('jinja2_templetes/templetes_Q11.html',mode='w') as f:
        f.write(str(html))
    ...
    driver.get('file:///Users/t_sasaki/Documents/%E6%A5%AD%E5%8B%99/%E3%82%A2%E3%83%B3%E3%82%B1%E3%83%BC%E3%83%88/%E3%82%AB%E3%83%9F%E3%82%AA%E3%83%B3/macro_mk/jinja2_templetes/templetes_Q11.html')
    page_width = driver.execute_script('return document.body.scrollWidth')
    page_height = driver.execute_script('return document.body.scrollHeight')
    driver.set_window_size(page_width,page_height)
    driver.save_screenshot('save_images/Q{}_single_text.png'.format(select))
    driver.quit()
    ...
    # 貼り付ける画像の読み込み
    fnms = ['./save_images/Q{}_single_text.png'.format(select)]
    # ファイルでループさせる
    for fnm in fnms:
        # スライドの追加。６は白紙のスライド
